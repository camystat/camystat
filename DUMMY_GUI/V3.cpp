...
class SavgolWindowLengthNotOddException : public std::invalid_argument {
public:
	SavgolWindowLengthNotOddException() : std::invalid_argument("Window length must be odd") {}
};

class SavgolPolyorderNotLessThanWindowLengthException : public std::invalid_argument {
public:
	SavgolPolyorderNotLessThanWindowLengthException(size_t polyorder, size_t window_length) : std::invalid_argument("Polynomial order (received: " + std::to_string(polyorder) + ") must be less than the window length(received: " + std::to_string(window_length) + ")") {}
};

class SavgolWindowLengthNotLTEDataLengthException : public std::invalid_argument {
public:
	SavgolWindowLengthNotLTEDataLengthException(size_t window_length, size_t data_length) : std::invalid_argument("Window length (received: " + std::to_string(window_length) + ") must be <= length of data (which is: " + std::to_string(data_length) + ")") {}
};

/// <summary>
/// Applies a Savitzky-Golay filter to the given data.
/// </summary>
/// <param name="data">The data to be filtered</param>
/// <param name="window_length">The length of the filter window</param>
/// <param name="polyorder">The order of the polynomial</param>
/// <returns>The filtered data (new object)</returns>
std::vector<double> savgolFilter(const std::vector<double>& data, size_t window_length, size_t polyorder) {
	// Ensure window length is odd and greater than polyorder
	if (window_length % 2 == 0) {
		throw SavgolWindowLengthNotOddException();
	}

	if (window_length > data.size()) {
		throw SavgolWindowLengthNotLTEDataLengthException(window_length, data.size());
	}

	if (polyorder >= window_length) {
		throw SavgolPolyorderNotLessThanWindowLengthException(polyorder, window_length);
	}
	...
	for (size_t i = 0; i < data.size(); ++i) {
		...
		for (int j = -half_window; j <= half_window; ++j) {
			size_t idx = std::clamp<size_t>(i + j, 0, data.size() - 1);
...
/// <summary>
/// Applies a Savitzky-Golay filter to the given data, presenting any errors to the user visually if they occur
/// </summary>
/// <param name="data"></param>
/// <param name="window_length"></param>
/// <param name="polyorder"></param>
/// <returns></returns>
std::vector<double> V3::Smoothing::applySavgolFilter(const std::vector<double>& data, size_t window_length, size_t polyorder) {
	try {
		...
		wxMessageDialog dialog(NULL, "Savgol: " + std::string(e.what()), wxMessageBoxCaptionStr, wxOK | wxCENTER | wxDIALOG_NO_PARENT);
		dialog.ShowModal();
		throw;
	}
}
...
/// <summary>
/// Smoothes the given values using an iterative smoothing algorithm using adaptive threshold based on the percentile of differences in consecutive values.
/// Values are smoothed by averaging values that deviate significantly from their neighbors.
/// </summary>
/// <param name="values">The values to be smoothed</param>
/// <param name="percentile">The percentile of differences to use as a threshold for smoothing</param>
/// <returns>The smoothed values (new object)</returns>
std::vector<double> V3::Smoothing::smoothValues(const std::vector<double>& values, float percentile) {
	if (values.size() < 3) {
