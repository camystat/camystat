...
def plot_video_events(values_path, events_path, video_name, save_path, fps, path_to_remove, normalize_flag):
...
def plot_auto_binarization_threshold(xor_results_path: str, threshold_result_path: str, save_path: str):
	...
	plt.axvline(x=int(threshold), color='r', linestyle='--', label=f"Threshold: {threshold}")
	plt.legend()
	plt.grid()
...
if __name__ == "__main__":
	...
	parser_video_events = subparsers.add_parser('video_events', help='Plot video events.')
	parser_video_events.add_argument('values_path', type=str, help='Path to the CSV file containing the values.')
	parser_video_events.add_argument('events_path', type=str, help='Path to the CSV file containing the events.')
	parser_video_events.add_argument('video_name', type=str, help='Name of the video.')
	parser_video_events.add_argument('save_path', type=str, help='Path to save the plot.')
	parser_video_events.add_argument('fps', type=int, help='Frames per second of the video.')
	parser_video_events.add_argument('path_to_remove', type=str, help='Path to remove.')
	parser_video_events.add_argument('normalize_flag', type=int, help='Normalize the values.', default=False, nargs='?')
	
	parser_auto_binarization_threshold = subparsers.add_parser('auto_binarization_thresh', help='Plot automatic binarization threshold results.')
	parser_auto_binarization_threshold.add_argument('xor_results_path', type=str, help='Path to the CSV file containing the XOR results for each threshold.')
	parser_auto_binarization_threshold.add_argument('threshold_result_path', type=str, help='Path to the TXT file containing the calculated threshold.')
	parser_auto_binarization_threshold.add_argument('save_path', type=str, help='Path to save the plot.')
	...
	while argv:
		...
		match options.subparser_name:
			case "video_events":
				...
				plot_video_events(values_path, events_path, video_name, save_path, fps, path_to_remove, normalize_flag)
			...
			case "auto_binarization_thresh":
				...
				plot_auto_binarization_threshold(xor_results_path, threshold_result_path, save_path)
